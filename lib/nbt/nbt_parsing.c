...
static void dump_long_array(const struct nbt_long_array ia, struct buffer* b)
{
    assert(ia.length >= 0);
...
static nbt_status dump_long_array_binary(const struct nbt_long_array ia, struct buffer* b)
{
    int32_t dumped_length = ia.length;
